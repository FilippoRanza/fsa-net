...
pub fn run(net: &network::Network, reqs: &command::Requests, conf: &super::EngineConfig) -> Vec<NetworkResult> {
    reqs.commands
        .iter()
...
fn run_request(net: &network::Network, req: &command::Command, conf: &super::EngineConfig) -> NetworkResult {
    match req {
